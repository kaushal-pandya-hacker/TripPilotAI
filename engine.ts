...
// Define the interface for the generated itinerary
...
export function getOpenAIClient(customKey?: string) {
...
export function generateOfflineItinerary(params: any): GeneratedItinerary {
  ...
  const includesLeh = destList.some((d: string) => d.toLowerCase().includes('leh') || d.toLowerCase().includes('ladakh'));
  const includesKedar = destList.some((d: string) => d.toLowerCase().includes('kedar'));
...
export async function generateItinerary(params: any, customKey?: string): Promise<GeneratedItinerary> {
  ...
  if (client) {
    try {
      ...
      return JSON.parse(resultText) as GeneratedItinerary;
    } catch (e) {
...
export async function editItinerary(currentItinerary: GeneratedItinerary, message: string, customKey?: string): Promise<GeneratedItinerary> {
  const command = message.toLowerCase();
  const client = getOpenAIClient(customKey);
  
  if (!client) {
    const updated = JSON.parse(JSON.stringify(currentItinerary)) as GeneratedItinerary;
  ...
  try {
    ...
    return JSON.parse(response.choices[0].message.content || '{}') as GeneratedItinerary;
  } catch (e) {
...
function getOfflineChatResponse(message: string): string {
...
export async function generalChat(message: string, history: Array<{ role: 'user' | 'assistant'; content: string }>, customKey?: string): Promise<{ text: string }> {
  const client = getOpenAIClient(customKey);
  if (client) {
    try {
      const messages = [
...
function getLehKedarEpicPreset(start: string, destinations: string[], days: number, budget: number, style: string, transport: string, rental: boolean): GeneratedItinerary {
  return {
...
function getDynamicGeneratedPreset(start: string, destinations: string[], days: number, budget: number, style: string, transport: string, rental: boolean): GeneratedItinerary {
